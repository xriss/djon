...
for fname in $files ; do
	...
	{ node -- ../js/djon_cmd.mjs "${fname}" 2>&1 | sed -n '/at djon.check_error/q;p' | sed '/RangeError:/q' | tr -c '[:print:]\t\n\r' '#' ; } &>>output.json
	echo "#js.djon" >>output.json
	{ node -- ../js/djon_cmd.mjs --djon "${fname}" 2>&1 | sed -n '/at djon.check_error/q;p' | sed '/RangeError:/q' | tr -c '[:print:]\t\n\r' '#' ; } &>>output.json
